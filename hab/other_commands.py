from config import Columns as Col, callsign
...
indices = {}
missing_indices = {}
optional_quantities = {'Packets':  1, 'Speed': 6, 'Heading': 7, 'Satellites': 8, 'Internal': 9, 'External': 10,
                       'Pressure': 11, 'Hum_Check': 12}
...
def record_launch_values(filepath):
    ...
    try:
        ...
        start_time = line[Col.TIME.value]
        start_lat = float(line[lat_index])
        start_long = float(line[long_index])
        start_alt = float(line[alt_index])

        if indices.get('External') is None or indices.get('Pressure') is None:
...
def add_telemetry(filepath):
    ...
    try:
        ...
        if len(indices) == 8:
            ...
            new_line = [line[callsign_index], line[packet_index], line[time_index], float(line[lat_index]),
                        float(line[long_index]),
                        float(line[alt_index]), float(line[speed_index]), float(line[heading_index]),
                        int(line[satellites_index]),
...
def false_telemetry(filepath, state=None):
    ...
    if state is None:  # For the case where state has not been defined yet
