...
def record_launch_values(filepath):
    ...
    try:
        ...
        return (safe_launch_values[2],safe_launch_values[3],safe_launch_values[4],safe_launch_values[5],safe_launch_values[10],safe_launch_values[11]) 
...
def add_telemetry(filepath):
    ...
    try:
        ...
        new_line = [line[callsign_index],line[packet_index],line[time_index], float(line[lat_index]),float(line[long_index]),\
                    ...
                    float(line[internal_index]),float(line[external_index]),float(line[pressure_index]),0,0]#,float(line[-2]),line[-1]]
